...
{
    valdesc_data_t *data = (valdesc_data_t *)user_data;
    UNUSED4(ctx, err, attribute_names, attribute_values);
    ...
    GError      **err)
{
    ...
    *state = SEARCHING_FOR_REGISTRY;
    ...
    result = g_markup_parse_context_parse(
        ctx, xml_data, xml_data_len, err);
    if (result) {
        result = g_markup_parse_context_end_parse(ctx, err);
    }
    ...
    GError              **err)
{
    ...
    if (strcmp(element_name, "name") == 0) {
        g_free((void *)data->ie.name);
        data->ie.name = g_strstrip(g_string_free(data->text, FALSE));
        data->text = g_string_sized_new(32);
...
{
    g_assert(xml_data);
    if (ipfix_mappings_parse(xml_data, xml_data_len, err)) {
        return ipfix_elements_parse(model, xml_data, xml_data_len, err);
    }
    return FALSE;
}
    ...
    GError        **err)
{
    ...
    if (g_file_get_contents(filename, &buffer, &len, err)) {
        gboolean rv = fbInfoModelReadXMLData(model, buffer, len, err);
        g_free(buffer);
        return rv;
    }
    return FALSE;
