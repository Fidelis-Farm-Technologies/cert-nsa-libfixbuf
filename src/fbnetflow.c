/*
 *  Copyright 2008-2023 Carnegie Mellon University
...
/*
 *  Copies `value` into `ptr` doing host to network translation if needed.
 */
...
/*
 *  Moves `ptr` forward the size of a uint16_t. The second param is ignored.
 *
 *  Alternatively, a version of READU16INC() that does not read.
 */
#define SKIPU16INC(ptr, ignored) {                  \
        uint16_t *ru16_t16ptr;                      \
        ptr += sizeof(*ru16_t16ptr) / sizeof(*ptr); \
    }

#define SKIPU32INC(ptr, ignored) {                  \
        uint32_t *ru32_t32ptr;                      \
        ptr += sizeof(*ru32_t32ptr) / sizeof(*ptr); \
    }
    ...
    GError             **err)
{
    ...
    transState->sysUpTime = htonll(transState->sysUpTime);
    ...
    while (lengthParsed < recLength) {
        ...
        if (!optLen && !optScopeLen) {
            g_set_error(err, FB_ERROR_DOMAIN, FB_ERROR_NETFLOWV9,
                        ...
                        "(optScopeLen %u, optLen %u).", optScopeLen,
                        optLen);
            return 0;
        }
...
fbCollectorPostProcV9(
    ...
    GError        **err)
{
    /* uint32_t  timeStamp; -- ignored by SKIPU32INC() */
    uint16_t  recordCount;
...
#if FB_NETFLOW_DEBUG == 1
    /* no need to read value that we are about to overwrite */
    uint16_t version;
    ...
    SKIPU32INC(msgOsetPtr, timeStamp);
    ...
    GError        **err)
{
    ...
    return fbCollectorSetTranslator(collector,
                                    fbCollectorPostProcV9,
                                    fbCollectorDecodeV9MsgVL,
                                    fbCollectorMessageHeaderV9,
                                    fbCollectorTransCloseV9,
